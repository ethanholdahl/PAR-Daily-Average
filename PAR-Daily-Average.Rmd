...
``` {r input panel, cache=FALSE, echo=FALSE}
inputPanel(
  ...
  sliderInput("time", label = "time:",
              min = 0, max = 2, step = 1/800, value = 1)
)
```

The colorful plot represents the actual elevation, the transparent grey plot represents the elevation that would make Wang's algorithm equivalent to my ratio algorithm.

```{r output elevation, cache=FALSE, echo=FALSE}
reactive(elevation(input$time, input$Latitude, input$Longitude, input$year, input$day))

renderPlot({
  ggplot(data = NULL, aes(x = 24*60*60*t, y = sin(elevation(t,input$Latitude,input$Longitude,input$year,input$day)*pi/180), color = sin(elevation(t,input$Latitude,input$Longitude,input$year,input$day)*pi/180), size = 3, alpha = 1))+
    ...
    geom_point()+
    geom_point(data = NULL, aes(x = 24*60*60*t[t>Sunrise & t<Sunset], y = sin((t[t>Sunrise & t<Sunset]-Sunrise)*pi/(Sunset-Sunrise)), color = -.2, size = 3, alpha = .2)) +
geom_point(data = NULL, aes(x = 24*60*60*(t[t>Sunrise & t<Sunset]-1), y = sin((t[t>Sunrise & t<Sunset]-Sunrise)*pi/(Sunset-Sunrise)), color = -.2, size = 3, alpha = .2)) +
    geom_point(data = NULL, aes(x = 24*60*60*(t[t>Sunrise & t<Sunset]+1), y = sin((t[t>Sunrise & t<Sunset]-Sunrise)*pi/(Sunset-Sunrise)), color = -.2, size = 3, alpha = .2)) +
    scale_x_time(limits = c(0,2*24*60*60))
})
```

```{r plot 2, cache=FALSE, echo=FALSE}
