...
function(input, output, session) {
  ...
  observe({
    ...
    # output$a2 = renderPrint({url})
    
    # "depth" is how many levels the url in the query string is
    depth <- function(x)
      length(unlist(strsplit(x, "/")))
    
    # output$a3 = renderPrint({length(values$myurl)})
    # output$a4 = renderPrint({depth(url)})
    # output$a5 = renderPrint({input[[values$parent_tab]]})
    # output$a6 = renderPrint({tail(values$myurl, 1)})
    #
  ...
  url3 <-
    a("GitHub", href = "https://github.com/ethanholdahl/PAR-Daily-Average")
  ...
  elevation = function(t, Latitude, Longitude, year, day) {
    ...
    GeomMeanLongSunDeg = (280.46646 + JulianCentury * (36000.76983 + JulianCentury *
                                                         .0003032)) %% 360
    
    GeomMeanAnomSunDeg = 357.52911 + JulianCentury * (35999.05029 - 0.0001537 *
                                                        JulianCentury)
    
    EccentEarthOrbit = 0.016708634 - JulianCentury * (0.000042037 + 0.0000001267 *
                                                        JulianCentury)
    
    SunEqOfCtr = sin(pi / 180 * (GeomMeanAnomSunDeg)) * (1.914602 - JulianCentury *
                                                           (0.004817 + 0.000014 * JulianCentury)) + sin(pi / 180 * (2 * GeomMeanAnomSunDeg)) *
      (0.019993 - 0.000101 * JulianCentury) + sin(pi / 180 * (3 * GeomMeanAnomSunDeg)) *
      0.000289
    
    SunTrueLongDeg = GeomMeanLongSunDeg + SunEqOfCtr
    
    SunAppLongDeg = SunTrueLongDeg - 0.00569 - 0.00478 * sin(pi / 180 *
                                                               (125.04 - 1934.136 * JulianCentury))
    
    MeanObliqEclipticDeg = 23 + (26 + ((
      21.448 - JulianCentury * (46.815 + JulianCentury * (0.00059 - JulianCentury *
                                                            0.001813))
    )) / 60) / 60
    
    ObliqCorrDeg = MeanObliqEclipticDeg + 0.00256 * cos(pi / 180 * (125.04 -
                                                                      1934.136 * JulianCentury))
    
    SunDeclineDeg = 180 / pi * (asin(sin(pi / 180 * (ObliqCorrDeg)) * sin(pi /
                                                                            180 * (SunAppLongDeg))))
    
    vary = tan(pi / 180 * (ObliqCorrDeg / 2)) * tan(pi / 180 * (ObliqCorrDeg /
                                                                  2))
    
    EqOfTimeMinutes = 4 * 180 / pi * (
      vary * sin(2 * pi / 180 * (GeomMeanLongSunDeg)) - 2 * EccentEarthOrbit *
        sin(pi / 180 * (GeomMeanAnomSunDeg)) + 4 * EccentEarthOrbit * vary * sin(pi /
                                                                                   180 * (GeomMeanAnomSunDeg)) * cos(2 * pi / 180 * (GeomMeanLongSunDeg)) -
        0.5 * vary * vary * sin(4 * pi / 180 * (GeomMeanLongSunDeg)) - 1.25 * EccentEarthOrbit *
        EccentEarthOrbit * sin(2 * pi / 180 * (GeomMeanAnomSunDeg))
    )
    ...
    HAval = cos(pi / 180 * (srssangle)) / (cos(pi / 180 * (Latitude)) *
                                             cos(pi / 180 * (SunDeclineDeg))) - tan(pi / 180 * (Latitude)) * tan(pi /
                                                                                                                   180 * (SunDeclineDeg))
    ...
    Sunrise <<-
      ((SolarNoon * 1440 - HASunriseDeg * 4) / 1440) %% 1
    #keep sunrise positive and between zero and 1
    Sunset <<-
      Sunrise + SunlightDuration
    #assign SR/SS to global for use in other functions
    
    C = EqOfTimeMinutes / 4 + Longitude
    
    
    return(90 - (acos(A + B * cos((t * 360 + C + 180) * pi / 180
    ))) * 180 / pi)
  ...
  add_wang = function(t, Latitude, Longitude, year, day) {
    ...
    time_ele_PAR = time_ele_PAR %>%
      select(time = t) %>%
      mutate(
        observation = 0 == (time * 8 * precision) %% (precision),
  ...
  add_ratio = function(t, Latitude, Longitude, year, day) {
    ...
    ratio_calc = time_ele_PAR %>%
      mutate(
        ...
        linear_ratio =  rat1 * (time2 - time) / (time2 - time1) + rat2 *
  ...
  output$elevation = renderPlot({
    ggplot(data = NULL,
      ...
      labs(
        ...
        y = "sin(elevation)",
        color = "sin(ele)",
        title = "Solar elevation for selected inputs.",
        caption = str_wrap(
          "The large purple point represents an observation at the specified time
           The purple line shows the elevation under which Wang's algorithm yields a linear relationship in interpolated PAR ratio between observations.",
          width = 80
        )
      ) +
      theme(
  ...
  output$PAR = renderPlot({
    ...
    if (input$Wang)
      p = p + geom_line(data = time_ele_PAR(), aes(x = time, y = wang_PAR, color = wang_ratio))
    if (input$ratio)
      p = p + geom_line(data = time_ele_PAR(), aes(x = time, y = linear_PAR, color = linear_ratio))
  ...
  output$ratio = renderPlot({
    p = ggplot(data = observations(), aes(x = time, y = ratioMax, color = PAR)) +
      ...
      labs(
        y = "Ratio",
        title = "Interpolated PAR ratio",
        caption = str_wrap(
          "The points represent observations. The colorful line represents the ratio of the interpolated PAR to the theoretical maximum PAR at that elevation. The color of the line represents the interpolated PAR values.",
          width = 80
        )
      ) +
      theme(
    ...
    if (input$Wang)
      p = p + geom_line(data = time_ele_PAR(), aes(x = time, y = wang_ratio, color = wang_PAR))
    if (input$ratio)
      p = p + geom_line(data = time_ele_PAR(), aes(x = time, y = linear_ratio, color = linear_PAR))
