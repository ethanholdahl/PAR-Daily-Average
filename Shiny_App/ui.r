...
fluidPage(
  ...
  tags$head(
    HTML(
      "<title> Photosynthetically Active Radiation (PAR) by Ethan Holdahl </title>"
    )
  ),
  ...
  navbarPage(
    id = "bannerTabs",
    title = HTML(
      "Photosynthetically Active Radiation (PAR) by <a href='https://ethanholdahl.com'style='color: darkgreen; text-decoration: none'>Ethan Holdahl</a>"
    ),
    tabPanel(
      "Methods",
      value = "Methods",
      tags$body(
        ...
        h2("Introduction"),
        HTML(
          "In this document I will outline my methods used for calculating daily totals of photosynthetically active radiation (PAR)
        ...
        HTML(
          "We know that the amount of radiation emitted from the sun, the solar constant, is 1361 watts per square meter on a surface perpendicular
        ...
        h2("Ratio method"),
        HTML(
          "Generally, we can estimate the incident PAR for unobserved times by taking a weighted average of the PAR ratios of the nearest
        ...
        br(),
        HTML(
          "If there is only one observation in the day or if \\(\\small t\\) falls between sunrise and the first observation or between the last observation and
        ...
        br(),
        HTML(
          "So, to calculate PAR for a given day at a specified location with \\(\\small n\\) daylight observations we integrate \\(\\small InstPAR(t)\\)
        ...
        h2("Existing methods"),
        HTML(
          "There already exist methods for calculating daily PAR values. Liang et al.\\(^3\\) has published a book containing algorithms for a variety of different
        ...
        HTML(
          "Navigate to the <a href='https://shiny.ethanholdahl.com/PAR-Daily-Average/?url=Plots'style='color: darkgreen; text-decoration: none'>Plots page</a>
        ...
        HTML(
          "Refraction makes our calculations more difficult when the sun is near 0 degrees elevation. When radiation passes through the atmosphere it is
        ...
        h2("Remarks"),
        HTML(
          "Wang et al.'s algorithm is very good in the majority of cases because it is so similar to the ratio method. However it is not robust to extreme
    ...
    tabPanel(
      ...
      HTML(
        "You may select paramaters as well as which interpolation method to use on the input panel on the left. On the right, plots are depecting the 1. Solar Elevation 2. Interpolated PAR Values and 3. Interpolated PAR Ratio"
      ),
      br(),
      HTML(
    ...
    tabPanel(
      ...
      tabsetPanel(
        ...
        tabPanel("April 1", value = "April",
                 img(src = "April1.PNG")),
        tabPanel("July 1", value = "July",
                 img(src = "July1.PNG")),
        tabPanel("October 1", value = "October",
                 img(src = "Oct1.PNG"))
      ),
      br(),
      br(),
